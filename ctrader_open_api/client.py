...
class Client(ClientService):
    def __init__(
        ...
        defaultResponseTimeoutInSeconds=30,
    ):
        self._runningReactor = reactor
        self.numberOfMessagesToSendPerSecond = numberOfMessagesToSendPerSecond
        self.defaultResponseTimeoutInSeconds = defaultResponseTimeoutInSeconds
    ...
    def _received(self, message):
        ...
        if message.clientMsgId is not None and message.clientMsgId in self._responseDeferreds:
            ...
            self._responseDeferreds.pop(message.clientMsgId, None)
            responseDeferred.callback(message)

    def send(self, message, clientMsgId=None, responseTimeoutInSeconds=None, **params):
        if type(message) in [str, int]:
            message = Protobuf.get(message, **params)

        if responseTimeoutInSeconds is None:
            responseTimeoutInSeconds = self.defaultResponseTimeoutInSeconds
        ...
        responseDeferred.addErrback(
            lambda failure: self._onResponseFailure(failure, clientMsgId)
        )

        if responseTimeoutInSeconds and responseTimeoutInSeconds > 0:
            responseDeferred.addTimeout(
                responseTimeoutInSeconds,
                self._runningReactor,
            )

        protocolDiferred = self.whenConnected(failAfterFailures=1)
        protocolDiferred.addCallbacks(
            lambda protocol: protocol.send(
                message,
