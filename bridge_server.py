...
class MT5BridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
              ...
              try:
                  ...
                  logger.info(f"Received trade event: {data.get('event_type')} for ticket {data.get('ticket')}")
            ...
            self.end_headers()
            response = {"status": "success", "message": "Trade event processed"}
    ...
    def do_GET(self):
        ...
        if self.path == "/health":
                      ...
                      self.send_header("Content-Type", "application/json")
                      self.end_headers()
                      response = {"status": "ok", "service": "MT5 to cTrader Bridge"}
                      self.wfile.write(json.dumps(response).encode("utf-8"))
else:
