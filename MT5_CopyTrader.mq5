...
void SendOpenSignal(ulong ticket) {
   ...
   string jsonData = StringFormat(
      "{\"action\":\"OPEN\",\"ticket\":%d,\"symbol\":\"%s\",\"type\":\"%s\",\"volume\":%.2f,\"price\":%.5f,\"sl\":%.5f,\"tp\":%.5f,\"magic\":%d}",
      ticket, symbol, tradeType, volume, openPrice, sl, tp, magic
   );
...
void SendToServer(string jsonData) {
   ...
   if(res == -1) {
      ...
      Print("WebRequest error: ", error, ". Make sure URL is added to allowed URLs in Tools > Options > Expert Advisors");
      return;
   }
