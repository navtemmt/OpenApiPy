...
def handle_open_event(data, account_manager):
    ...
    # Copy to all configured accounts
...
def handle_modify_event(data, account_manager):
    ...
    # Apply modification to all accounts
    for account_name, client_config in account_manager.get_all_accounts():
              try:
            ...
            if position_id:
                ...
                # Position not yet mapped, store for later
                  logger.warning(
...
def handle_close_event(data, account_manager):
    ...
    # Close position in all accounts
    for account_name, client_config in account_manager.get_all_accounts():
              try:
                            position_id = account_manager.get_position_id(account_name, ticket)
