...
def _resolve_symbol_name_from_id(
    self,
    ...
    try:
        symbol = (
            self.symbol_details.get(int(symbol_id))
            if hasattr(self, "symbol_details")
            else None
        )
...
def _extract_response_context(
    ...
    order_trade_data = _read_attr_or_key(
        order,
        "tradeData",
        None,
    )

    position_trade_data = _read_attr_or_key(
        position,
        "tradeData",
        None,
    )
    ...
    ticket = (
        _parse_mt5_ticket_from_label(label)
        if label
        else None
    )
    ...
    return {
        ...
        "label": label,
        "execution_type": _read_attr_or_key(
            extracted,
            "executionType",
            None,
        ),
        "order_id": _read_attr_or_key(
            order,
            "orderId",
            None,
        ),
        "position_id": _read_attr_or_key(
            position,
            "positionId",
            None,
        ),
        "deal_id": _read_attr_or_key(
            deal,
            "dealId",
            None,
        ),
...
def _get_close_volume_policy(self) -> str:
    """Return the configured per-account close sizing policy."""
    ...
    if policy not in allowed:
        logger.warning(
            "Unknown close_volume_policy=%r; "
            "falling back to min_volume",
            policy,
        )
        return "min_volume"
...
def send_market_order(
    ...
    try:
        ...
        if int(volume) <= 0:
            raise ValueError(
                "Market order volume must be "
                f"greater than zero: {volume}"
            )

        if sl is not None and float(sl) > 0:
            sl = self.round_price_for_symbol(
                symbol_id,
                float(sl),
            )

        if tp is not None and float(tp) > 0:
            tp = self.round_price_for_symbol(
                symbol_id,
                float(tp),
            )
    ...
    logger.info(
        "Sending market order accountId=%s "
        "ticket=%s symbol=%s symbolId=%s "
        "side=%s volume=%s sl=%s tp=%s label=%s",
        account_id,
        ticket,
        symbol_name,
        symbol_id,
    ...
    def on_resp(result):
        try:
            ...
            _notify_response_warn(
                ...
                "Failed to parse market order response; "
                "raw response logged",
                {
                    **_base_event_context(
                        account_id=account_id,
            ...
            logger.warning(
                "Order response raw accountId=%s "
                "ticket=%s symbol=%s symbolId=%s "
                "label=%s raw=%r",
                account_id,
                ticket,
...
def send_pending_order(
    ...
    try:
        ...
        if stop_price > 0:
            stop_price = self.round_price_for_symbol(
                symbol_id,
                stop_price,
            )

        if limit_price > 0:
            limit_price = self.round_price_for_symbol(
                symbol_id,
                limit_price,
            )
    ...
    if ptype == "limit":
        ...
        if stop_price <= 0.0:
            raise ValueError(
                "STOP_LIMIT pending order requires "
                "stop_price > 0"
            )
    ...
    logger.info(
        "Sending pending order accountId=%s "
        "ticket=%s symbol=%s symbolId=%s "
        "type=%s side=%s vol=%s stop=%s "
        "limit=%s SL=%s TP=%s exp=%s label=%s",
        account_id,
        ticket,
        symbol_name,
        symbol_id,
    ...
    def on_resp(result):
        try:
            ...
            _notify_response_warn(
                ...
                "Failed to parse pending order response; "
                "raw response logged",
                {
                    **_base_event_context(
                        account_id=account_id,
            ...
            logger.warning(
                "Pending order response raw "
                "accountId=%s ticket=%s symbol=%s "
                "symbolId=%s label=%s raw=%r",
                account_id,
                ticket,
                symbol_name,
                symbol_id,
...
def amend_pending_order(
    ...
    try:
        ...
        if stop_loss is not None and float(stop_loss) > 0:
            stop_loss = self.round_price_for_symbol(
                symbol_id,
                float(stop_loss),
            )
        else:
        ...
        if take_profit is not None and float(take_profit) > 0:
            take_profit = self.round_price_for_symbol(
                symbol_id,
                float(take_profit),
            )
        else:
    ...
    def on_resp(result):
        try:
            ...
            logger.info(
                "Amend pending order response %s %s",
                _format_context(context),
                extracted,
            )
            ...
            _notify_response_warn(
                ...
                "Failed to parse amend pending response; "
                "raw response logged",
                {
                    **_base_event_context(
                        account_id=account_id,
            ...
            logger.warning(
                "Amend pending order response raw "
                "accountId=%s orderId=%s symbol=%s "
                "symbolId=%s raw=%r",
                account_id,
                order_id,
                symbol_name,
...
def cancel_pending_order(
    ...
    logger.info(
        "Cancelling pending order accountId=%s "
        "orderId=%s",
        account_id,
        order_id,
    )
    ...
    def on_resp(result):
        try:
            ...
            _notify_response_warn(
                ...
                "Failed to parse cancel pending response; "
                "raw response logged",
                {
                    **_base_event_context(
                        account_id=account_id,
            ...
            logger.warning(
                "Cancel order response raw "
                "accountId=%s orderId=%s raw=%r",
                account_id,
                order_id,
                result,
            )
...
def modify_position(
    ...
    symbol_name = _resolve_symbol_name_from_id(
        self,
        symbol_id if symbol_id is not None else None,
    )
    ...
    logger.info(
        "Modifying position accountId=%s "
        "positionId=%s symbol=%s symbolId=%s "
        "SL=%s->%s TP=%s->%s",
        account_id,
        position_id,
        symbol_name,
    ...
    def on_resp(result):
        try:
            ...
            _notify_response_warn(
                ...
                "Failed to parse modify position response; "
                "raw response logged",
                {
                    **_base_event_context(
                        account_id=account_id,
            ...
            logger.warning(
                "Amend response raw accountId=%s "
                "positionId=%s symbol=%s symbolId=%s "
                "raw=%r",
                account_id,
                position_id,
...
def close_position(
    ...
    **kwargs: Any,
    ...
    Supported keyword aliases:
      account_id
      position_id / pos_id / position
      volume / qty / volume_cents
      symbol_id

    Positional compatibility:
      close_position(account_id, position_id, volume, symbol_id)

    ``symbol_id`` is optional because ProtoOAClosePositionReq only requires
    accountId, positionId and volume. It is used here for broker volume
    snapping and symbol-aware logging when available.
    """
    ...
    if volume is None:
        volume = kwargs.get("qty")

    if volume is None:
        volume = kwargs.get("volume_cents")
    ...
    if (
        ...
        exc = TypeError(
            "close_position requires "
            "account_id, position_id, and volume"
        )
    ...
    try:
        ...
        if symbol_id is not None:
            ...
            policy = _get_close_volume_policy(
                self
            )

            snapped_volume = int(
                self.snap_volume_for_symbol(
                    symbol_id,
                    requested_volume,
                )
            )
            ...
            if snapped_volume <= 0:
                raise ValueError(
                    "Broker snapped close volume "
                    "to zero: "
                    f"requested={requested_volume}, "
                    f"snapped={snapped_volume}, "
                    f"symbolId={symbol_id}"
                )

            if (
                policy == "floor"
                and snapped_volume > requested_volume
            ):
                raise ValueError(
                    "Close volume would be increased "
                    "by broker snapping: "
                    f"requested={requested_volume}, "
                    f"snapped={snapped_volume}, "
                    f"symbolId={symbol_id}, "
            ...
            if (
                policy == "full_if_below_min"
                and snapped_volume > requested_volume
            ):
                logger.warning(
                    "Close size below broker minimum; "
                    "min-volume close permitted by policy. "
    ...
    def on_resp(result):
        try:
            ...
            logger.warning(
                "Close response raw accountId=%s "
                "positionId=%s symbol=%s symbolId=%s "
                "volume=%s raw=%r",
                account_id,
                position_id,
