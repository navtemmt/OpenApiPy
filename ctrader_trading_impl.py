...
symbol_name = (
    _resolve_symbol_name_from_id(self, symbol_id)
...
logger.info(
    "Sending market order accountId=%s ticket=%s symbol=%s "
    "symbolId=%s side=%s volume=%s sl=%s tp=%s label=%s",
    account_id,
    ticket,
    symbol_name,
    symbol_id,
...
def on_resp(result):
    try:
        ...
        logger.warning(
            "Order response raw accountId=%s ticket=%s "
            "symbol=%s symbolId=%s label=%s raw=%r",
            account_id,
            ticket,
            symbol_name,
            symbol_id,
...
if ptype == "limit":
    if not limit_price or float(limit_price) <= 0.0:
        raise ValueError(
            "LIMIT pending order requires limit_price > 0"
        )
    ...
    if not stop_price or float(stop_price) <= 0.0:
        raise ValueError(
            "STOP pending order requires stop_price > 0"
        )
    ...
    if not stop_price or float(stop_price) <= 0.0:
        raise ValueError(
            "STOP_LIMIT pending order requires stop_price > 0"
        )

    if not limit_price or float(limit_price) <= 0.0:
        raise ValueError(
            "STOP_LIMIT pending order requires limit_price > 0"
        )
...
logger.info(
    "Sending pending order accountId=%s ticket=%s symbol=%s "
    "symbolId=%s type=%s side=%s vol=%s stop=%s limit=%s "
    "SL=%s TP=%s exp=%s label=%s",
    account_id,
    ticket,
    symbol_name,
    symbol_id,
...
def on_resp(result):
    try:
        ...
        logger.warning(
            "Pending order response raw accountId=%s ticket=%s "
            "symbol=%s symbolId=%s label=%s raw=%r",
            account_id,
            ticket,
            symbol_name,
...
if ptype == "limit":
    ...
    if stop_price is None:
        raise ValueError(
            "STOP_LIMIT amend requires stop_price > 0"
        )

    if limit_price is None:
        raise ValueError(
            "STOP_LIMIT amend requires limit_price > 0"
        )
...
logger.info(
    "Amending pending order accountId=%s orderId=%s symbol=%s "
    "symbolId=%s type=%s side=%s vol=%s stop=%s limit=%s "
    "SL=%s TP=%s exp=%s",
    account_id,
    order_id,
...
def on_resp(result):
    try:
        ...
        logger.warning(
            "Amend pending order response raw accountId=%s "
            "orderId=%s symbol=%s symbolId=%s raw=%r",
            account_id,
            order_id,
            symbol_name,
            symbol_id,
...
logger.info(
    "Cancelling pending order accountId=%s orderId=%s",
    account_id,
    order_id,
)
...
def on_resp(result):
    try:
        ...
        logger.warning(
            "Cancel order response raw accountId=%s "
            "orderId=%s raw=%r",
            account_id,
            order_id,
            result,
...
logger.info(
    "Modifying position accountId=%s positionId=%s symbol=%s "
    "symbolId=%s SL=%s->%s TP=%s->%s",
    account_id,
    position_id,
    symbol_name,
    symbol_id,
...
def on_resp(result):
    try:
        ...
        logger.warning(
            "Amend response raw accountId=%s positionId=%s "
            "symbol=%s symbolId=%s raw=%r",
            account_id,
            position_id,
            symbol_name,
            symbol_id,
...
IMPORTANT:
`position_id` resolution deliberately uses chained lookups instead
of dict.get() with multiple defaults. Python dict.get() accepts only
(key, default), so this must NOT be written as:

    kwargs.get("position_id", kwargs.get("pos_id"), kwargs.get("position"))
"""
...
#
# Use chained fallback instead.
# ------------------------------------------------------------
account_id = kwargs.get("account_id")

position_id = (
    kwargs.get("position_id")
    or kwargs.get("pos_id")
    or kwargs.get("position")
)

volume = (
    kwargs.get("volume")
    or kwargs.get("qty")
    or kwargs.get("volume_cents")
)
...
#
# symbol_id remains optional because the close request itself
# only requires accountId, positionId and volume.
# ------------------------------------------------------------
if account_id is None or position_id is None or volume is None:
    exc = TypeError(
        "close_position requires account_id, position_id, and volume"
    )
...
try:
    ...
    if volume <= 0:
        raise ValueError(
            f"Close volume must be greater than zero: {volume}"
        )
    ...
    if symbol_id is not None:
        ...
        if snapped_volume <= 0:
            raise ValueError(
                f"Broker snapped close volume to zero: "
...
logger.info(
    "Closing position accountId=%s positionId=%s "
    "symbol=%s symbolId=%s volume=%s",
    account_id,
    position_id,
    symbol_name,
    symbol_id,
...
def on_resp(result):
    try:
        ...
        logger.warning(
            "Close response raw accountId=%s positionId=%s "
            "symbol=%s symbolId=%s volume=%s raw=%r",
            account_id,
            position_id,
            symbol_name,
            symbol_id,
