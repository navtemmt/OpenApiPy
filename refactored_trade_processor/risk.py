...
def estimate_risk_ccy_per_1lot_from_symbol(
symbol,
entry_price: float,
sl_price: float,
) -> float:
...
def estimate_risk_ccy_per_1lot_from_mt5(
data: dict,
entry_price: float,
sl_price: float,
) -> float:
...
```
    ...
    tick_size = first_positive_float(
    ...
    tick_value = first_positive_float(
    ...
    quote_to_deposit = first_positive_float(
...
risk_per_1lot = (
    estimate_risk_ccy_per_1lot_from_mt5(
        mt5_data,
        entry,
        sl,
    )
...
if rm == "FIXED_USD":
    ...
    ref_amt = get_account_equity_or_balance(
        account_manager,
        account_name,
        config,
    )
...
def resolve_open_volume_for_account(
data: dict,
config,
*,
account_name=None,
...
risk_mode_value = risk_mode(config)
...
if risk_mode_value in (
    ...
    if not (
        ...
        return (
            None,
            f"REJECT_{risk_mode_value}_MISSING_CONTEXT",
        )
